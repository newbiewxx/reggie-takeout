...
│   ├── SetmealController.java       # 套餐管理：分页/增删改查/批量起停售（含套餐内菜品）
│   ├── UserController.java          # C 端用户：发送验证码/登录
...
│   ├── SetmealService.java          #   → SetmealServiceImpl（saveWithDishes / updateWithDishes / deleteWithDishes）
│   ├── SetmealDishService.java      #   → SetmealDishServiceImpl
...
│   └── SetmealDto.java              # 套餐 DTO（扩展 Setmeal，增加 categoryName、setmealDishes）
├── utils/
│   └── ValidateCodeUtils.java       # 验证码生成工具（4 位或 6 位数字）
└── exception/
...
### 用户上下文
- **BaseContext**：基于 `RequestContextHolder` + `HttpSession`
  - B 端员工：`getCurrentEmployeeId` / `setCurrentEmployeeId` / `removeCurrentEmployeeId`
  - C 端用户：`getCurrentUserId` / `setCurrentUserId` / `removeCurrentUserId`
...
- **LoginCheckFilter**：同时校验 B 端员工和 C 端用户登录状态，放行 `/user/sendMsg`、`/user/login`
...
### C 端用户登录
- `POST /user/sendMsg` — 发送验证码，生成 4 位数字存入 Session（key=手机号）
- `POST /user/login` — 验证码登录，校验 Session 中的验证码，新用户自动注册
- 验证码工具类：`ValidateCodeUtils.generateValidateCode(4)`
